...
- **Per-Server Wipe Schedules** *(coverage in progress)*: For servers listed in the shared GameMonitoring catalogue — currently official and well-known community servers — AutoConnect also picks up the operator's own posted wipe time and arms the connection ahead of it automatically. Any server outside that catalogue falls back to the manual "wait for restart" toggle; broader automatic coverage is planned.
